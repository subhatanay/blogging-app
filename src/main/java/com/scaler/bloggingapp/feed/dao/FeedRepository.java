...
public interface FeedRepository extends JpaRepository<ArticleEntity, Long> {
    ...
    @Query(nativeQuery = true,
            value = "SELECT art.article_id as articleId,subject, description,content,COALESCE(like_count, 0) as likesCount, art.create_timestamp as createdAt,us.user_id as userId , us.username as userName, us.full_name as fullName" +
                    " FROM articles art \n" +
                    "LEFT JOIN (SELECT article_id,COUNT(user_id) AS like_count FROM  likes li  GROUP BY article_id ) la \n" +
    ...
    public Page<TopFeedArticleContent> getTopFeeds(Pageable page);
